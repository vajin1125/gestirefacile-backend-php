...
if (!empty($_POST)) 
{
	...
	$sql = "UPDATE `users` SET `name` = '{$name}', `surname` = '{$surname}', `username` = '{$username}', `password` = '{$pwd}', `email` = '{$email}',  `enabled` = '{$enabled}', `tel` = '{$tel}', `cell` = '{$cell}', `max_business_num` = '{$maxbusinessnum}', `change_password` = '{$changepassword}', `image` = '{$image}'  WHERE `users`.`oid` = '{$oid}'";
	
	
	if(mysqli_query($con,$sql))
	{
		
		$sql = "DELETE FROM role_user_assoc where oid_user =  '{$oid}'";

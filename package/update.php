    ...
    $packageDetails = json_decode($_POST['package_details']);
		if (mysqli_query($con,$sql) && $packageDetails) {
			foreach ($packageDetails as $value)
			{
				...
				$sql = "INSERT INTO `package_detail`(`oid`,`oid_package`,`oid_resource_type`,`oid_category`,`oid_skill`,`oid_resource`,`qta`,`price`,`extra_descr`,`is_extra`,`note`,`hours`,`days`,`total_price`, `image`) VALUES ({$oidPackageDetail},{$oid}, {$oidResourceType}, {$oidCategory}, {$oidSkill}, {$oidResource}, {$value->qta}, {$price},'', 0,'{$value->note}',{$value->hours}, {$value->days}, {$value->total_price})";

...
if (!empty($_POST)) 
{
  ...
  if($_POST['image'] != '') {
	...
	$sql = "SELECT max(oid)+1 as id from packages ";
	...
	$sql = "INSERT INTO `packages` (`oid`, `name`, `descr`, `enabled`, `oid_user`, `total_price`, `image`) VALUES ('{$oid}','{$name}', '{$descr}', {$enabled}, {$oid_user}, {$total_price}, '{$image}')";
	if(mysqli_query($con,$sql))
	{
		...
		$extra_details = json_decode($_POST['extra_details']);
		if ($extra_details) {
